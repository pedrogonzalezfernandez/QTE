...
typedef struct _time_dev {
    ...
    // Stored data: eigenvalues (length n) and coefficients (length n)
    double        *eigenvalues;
...
void time_dev_list(t_time_dev *x, t_symbol *s, long argc, t_atom *argv);
...
void ext_main(void *r) {
    ...
    class_addmethod(c, (method)time_dev_list, "list", A_GIMME, 0);
...
void *time_dev_new(t_symbol *s, long argc, t_atom *argv) {
    ...
    if(x){
        ...
        // allocate arrays with default dimension
        x->eigenvalues = (double *)sysmem_newptrclear(x->n * sizeof(double));
        x->coeff       = (double complex *)sysmem_newptrclear(x->n * sizeof(double complex));
...
void time_dev_free(t_time_dev *x) {
    if (x->eigenvalues)
        sysmem_freeptr(x->eigenvalues);
    if (x->coeff)
        sysmem_freeptr(x->coeff);
}

void time_dev_assist(t_time_dev *x, void *b, long m, long a, char *s) {
    if(m==1)
        sprintf(s, "Input list: eigenvalues then coefficients; bang to compute");
    else
...
void time_dev_bang(t_time_dev *x) {
    ...
    if (!x->eigenvalues || !x->coeff) {
        object_error((t_object *)x, "No eigenvalues/coefficients stored. Send list first.");
        return;
    }
    ...
    for(long t = 0; t < tsteps; t++){
        ...
        for(long k = 0; k < n; k++){
            psi += x->coeff[k] * cexp(-I * x->eigenvalues[k] * time);
        }
        double mag = cabs(psi);
...
// Store eigenvalues and coefficients from a list message
// Expected format: n eigenvalues followed by n complex pairs (re, im)
void time_dev_list(t_time_dev *x, t_symbol *s, long argc, t_atom *argv) {
    long n = x->n;
    if (argc != n + 2*n) {
        object_post((t_object *)x, "Expected %ld numbers (n eigenvalues + 2*n coeffs)", n + 2*n);
        return;
    }
    // Resize storage if dimension changed
    if (!x->eigenvalues || !x->coeff) {
        // allocate if not done (should not happen)
        x->eigenvalues = (double *)sysmem_newptrclear(n * sizeof(double));
        x->coeff = (double complex *)sysmem_newptrclear(n * sizeof(double complex));
    }

    for (long i = 0; i < n; i++)
        x->eigenvalues[i] = atom_getfloat(argv + i);
    for (long i = 0; i < n; i++) {
        double re = atom_getfloat(argv + n + 2*i);
        double im = atom_getfloat(argv + n + 2*i + 1);
        x->coeff[i] = re + I * im;
    }
}
